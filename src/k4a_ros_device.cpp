...
static const std::unordered_map<k4a_color_resolution_t, std::string> color_mode_string = {
  { K4A_COLOR_RESOLUTION_720P, "720P" },   { K4A_COLOR_RESOLUTION_1080P, "1080P" },
  { K4A_COLOR_RESOLUTION_1440P, "1440P" }, { K4A_COLOR_RESOLUTION_1536P, "1536P" },
  { K4A_COLOR_RESOLUTION_2160P, "2160P" }, { K4A_COLOR_RESOLUTION_3072P, "3072P" },
};

static const std::unordered_map<k4a_depth_mode_t, std::string> depth_mode_string = {
  { K4A_DEPTH_MODE_NFOV_2X2BINNED, "NFOV_2X2BINNED" }, { K4A_DEPTH_MODE_NFOV_UNBINNED, "NFOV_UNBINNED" },
  { K4A_DEPTH_MODE_WFOV_2X2BINNED, "WFOV_2X2BINNED" }, { K4A_DEPTH_MODE_WFOV_UNBINNED, "WFOV_UNBINNED" },
...
#undef LIST_ENTRY
  ...
  // load calibration file from provided path or use default camera calibration URL at
  // $HOME/.ros/camera_info/<cname>.yaml
  ...
  const std::string calibration_url_rgb = params_.calibration_url.empty() ?
                                              std::string{} :
                                              params_.calibration_url + '/' + calibration_file_name_rgb + ".yaml";
  const std::string calibration_url_ir = params_.calibration_url.empty() ?
                                             std::string{} :
                                             params_.calibration_url + '/' + calibration_file_name_ir + ".yaml";

  ci_mngr_rgb_ = std::make_shared<camera_info_manager::CameraInfoManager>(node_rgb_, calibration_file_name_rgb,
                                                                          calibration_url_rgb);
  ci_mngr_ir_ =
...
k4a_result_t K4AROSDevice::renderDepthToROS(sensor_msgs::ImagePtr& depth_image, k4a::image& k4a_depth_frame)
{
  ...
  depth_image = cv_bridge::CvImage(std_msgs::Header(), encoding, depth_frame_buffer_mat).toImageMsg();
...
void K4AROSDevice::framePublisherThread()
{
  ...
  while (running_ && ros::ok() && !ros::isShuttingDown())
  {
    ...
    if (params_.depth_enabled)
    {
      ...
      if (params_.ir_enabled &&
          (ir_raw_publisher_.getNumSubscribers() > 0 || ir_bgr_publisher_.getNumSubscribers() > 0 ||
    ...
    if (params_.color_enabled)
    {
      ...
      else if (params_.color_format == "bgra")
      {
        ...
        if (params_.depth_enabled && params_.register_enabled &&
...
std::vector<std::string> K4AROSDevice::getKinectPorts()
{
  ...
  while (fgets(line, sizeof(line), lsusbPipe) != nullptr)
  {
    ...
    else if (token == "idVendor" && ss >> token)
    {
      current_id_vendor = token.substr(2);  // Remove "0x"
    }
    else if (token == "idProduct" && ss >> token)
    {
      current_id_product = token.substr(2);  // Remove "0x"
    }
    else if (token == "iProduct" && line_str.find("Azure Kinect") != std::string::npos)
    {
      ROS_INFO("Azure Kinect device found with Bus: %s, idVendor: %s, idProduct: %s", current_bus.c_str(),
               current_id_vendor.c_str(), current_id_product.c_str());
  ...
  std::vector<std::string> current_port_parts;  // To store the port path for each device level
  while (fgets(line, sizeof(line), lsusbTPipe) != nullptr)
  {
    std::string line_str = line;
    if (line_str.find("Port") != std::string::npos)
    {
      size_t pos = line_str.find("Port") + 5;
      size_t colon_pos = line_str.find(':', pos);
      if (colon_pos != std::string::npos)
      {
        std::string port_str = line_str.substr(pos, colon_pos - pos);
        // Clear higher levels of ports when depth changes (indicated by "|" count)
        size_t depth = std::count(line_str.begin(), line_str.end(), '|');
        if (depth < current_port_parts.size())
        {
          current_port_parts.resize(depth);
        }
        current_port_parts.push_back(port_str);
      }
    }
    else
    {
      ...
      {
        std::string search_string = "ID " + id_vendor[i] + ":" + id_product[i];
        if (line_str.find(search_string) != std::string::npos)
        {
          std::string full_port_path;
